...
clients, it is proposed that the APIs are configured to require client
credentials (e.g. a shared secret or a public key) and each API has an
associated access control list allowing calls only by trusted clients. This
approach is static, i.e. is the same for all users.
